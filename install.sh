...
if command -v realpath > /dev/null 2> /dev/null; then
	...
	echo "realpath is standard since POSIX 2024. Please get it installed and try again." >&2
	exit 1
fi
...
if [ -e "$FS_CUSTOMIZE_BASH_INSTALL_PROFILE" ] && grep -q -F ".bashrc" "$FS_CUSTOMIZE_BASH_INSTALL_PROFILE"; then
	echo "Found '.bashrc' in ${FS_CUSTOMIZE_BASH_INSTALL_PROFILE}, not modifying ${FS_CUSTOMIZE_BASH_INSTALL_PROFILE}."
	echo -e '\e[1;41mPlease confirm your $FS_CUSTOMIZE_BASH_INSTALL_PROFILE to check if "${HOME}/.bashrc" is sourced properly.\e[0;0m'
else
	if [ -n "$TERMUX_VERSION" ]; then
		if grep -q -F "/data/data/com.termux/files/home/.bashrc" "/data/data/com.termux/files/usr/etc/profile"; then
			echo "Note: Termux is source-ing ~/.bashrc in /data/data/com.termux/files/usr/etc/profile (at least in default install of some version)."
			...
			echo
			echo "You may examine /data/data/com.termux/files/usr/etc/profile to see if it loads your user-specific .bashrc. if so you may simply comment it out."
			echo
		fi
	fi
	...
	if ! [ -e "$FS_CUSTOMIZE_BASH_INSTALL_PROFILE" ]; then
		...
		echo | tee -a "${FS_CUSTOMIZE_BASH_INSTALL_PROFILE}"
		echo '# Typically .bashrc is sourced on a non-login shell and is not sourced on a login shell.' | tee -a "${FS_CUSTOMIZE_BASH_INSTALL_PROFILE}"
	...
	echo 'if [ -n "$BASH_VERSION" ] && [[ "$-" == *i* ]] && [ -r "$HOME/.bashrc" ]; then . "$HOME/.bashrc"; fi' | tee -a "${FS_CUSTOMIZE_BASH_INSTALL_PROFILE}"
	echo
fi
